...
<# Install Automate#>
$DownloadPath = "https://github.com/Underbee/Automate_Deploy/raw/main/Agent_Install.MSI"
...
$InstallExitCode = (Start-Process "msiexec.exe" -ArgumentList "/i $($SoftwarePath) /quiet /L*V $($LogFullPath)" -NoNewWindow -Wait -PassThru).ExitCode
If ($InstallExitCode -eq 0) {
    ...
    $InstallExitCode = (Start-Process "msiexec.exe" -ArgumentList "/i $($SoftwareFullPath) /quiet /L*V $($LogFullPath)" -NoNewWindow -Wait -PassThru).ExitCode
...
Else {
Write-Host "Unable to Complete NetFrame Work Installation Error: "$process.ExitCode
Exit
}
